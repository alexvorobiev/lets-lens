...
-- |
--
-- Lens (Map k v -> Maybe v -> Map k v) (Map k v -> Maybe v)
--
...
mapL k =
  Lens (\mkv v -> Map.alter (const v) k mkv) (Map.lookup k)

-- |
--
-- Lens (Set k -> Bool -> Set k) (Set k -> Bool)
--
...
product lab lcd =
  Lens 
    (\(a, c) (b, d) -> (set lab a b, set lcd c d))
    (\(a, d) -> (get lab a, get lcd d))
